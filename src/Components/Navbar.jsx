...
const Navbar = () => {
  const { user, setUser, signOutUserFunc, loading } =
    ...
    <nav className="bg-yellow-200 shadow-md px-4 md:px-8">
      <MyContainer className="flex items-center justify-between h-18">
        ...
        <ul className="hidden md:flex items-center gap-8 font-medium">
          ...
          <li><MyLink to="/available">Available Foods</MyLink></li>
        </ul>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Loading */}
          {loading ? (
            <ClockLoader size={32} color="#ff6712" />
          ) : user ? (
            /* User logged in → profile image for all screens */
            ...
            <>
              ...
              {/* Mobile Hamburger (only when user NOT logged in) */}
              <button
      ...
      {/* Mobile Dropdown Menu (only when user NOT logged in) */}
      {!user && isOpen && (
        <div className="md:hidden bg-base-100 shadow-lg rounded-lg mt-2 p-4">
          <ul className="flex flex-col gap-4 font-medium">
            ...
            <li>
              <Link to="/available" onClick={() => setIsOpen(false)}>
                Available Foods
              </Link>
            </li>
            <li>
